...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	for _, v in pairs(Config.DumpItems) do if not QBCore.Shared.Items[v.item] then print("^5Debug^7: ^2DumpItems^7: ^2Missing Item from ^4QBCore^7.^4Shared^7.^4Items^7: '^6"..v.."^7'") end end
end)
...
RegisterServerEvent('jim-recycle:Dumpsters:Reward', function()
	...
	for i = 1, math.random(1, 4) do
		...
		Player.Functions.AddItem(item.item, amount, nil, item.info)
		TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.item], 'add', amount)
		Citizen.Wait(100)
	end
